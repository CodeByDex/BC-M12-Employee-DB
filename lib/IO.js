...
async function DeleteDepartment(DepartmentID) {
    ...
    const [count] = await db.execute("SELECT COUNT(*) Count FROM Role WHERE DepartmentID = ?", [DepartmentID]);

    if(count[0].Count > 0) {
        console.log("Reassign Role Departments before deleting Department.");
        db.close();
        return 0;
    } else {
        const [results, fields] = await db.execute("DELETE FROM Department WHERE ID = ?", [DepartmentID]);
        db.close();
        return results.affectedRows;
    }
}
